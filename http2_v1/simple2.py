...
class Http2Connection:
        ...
        while True:
            try:
                ...
                if frame.stream_id > 0 and (frame.stream_id % 2) == 0:
                    await self.break_out_frame(0, StreamErrorCode.PROTOCOL_ERROR, self.writer)
                    continue
                ...
                if frame.stream_id > 0 and self.max_concurrent_streams < len(self.streams_in_receiving.keys()) + len(self.streams_in_que.keys()) + 1:
                    ...
                    await self.break_out_frame(0, StreamErrorCode.REFUSED_STREAM, self.writer)
                    continue
                ...
                if isinstance(frame, SettingsFrame):
                    ...
                    #     self.update_setting(frame)
                    #
                    #     ack_frame = SettingsFrame(flags=['ACK'])
                    #     self.writer.write(ack_frame.serialize())
                ...
                if isinstance(frame, HeadersFrame):
                    ...
                    if stream_id == frame.depends_on:
                        ...
                        await self.break_out_frame(stream_id, StreamErrorCode.PROTOCOL_ERROR, self.writer)
                        continue

                    if self.has_been_done(stream_id):
                        self.cancel_stream(stream_id)
                        await self.break_out_frame(stream_id, StreamErrorCode.STREAM_CLOSED, self.writer)
                        continue
                    ...
                    if self.max_frame_size < len(data):
                        self.cancel_stream(stream_id)
                        await self.break_out_frame(stream_id, StreamErrorCode.FRAME_SIZE_ERROR, self.writer)
                        continue
                    ...
                    if END_HEADERS in frame.flags:
                        try:
                            ...
                            await self.break_out_frame(0, StreamErrorCode.COMPRESSION_ERROR, self.writer)
                        else:
                ...
                print(e)
                if isinstance(frame, SettingsFrame):
                    await self.break_out_frame(0, StreamErrorCode.FRAME_SIZE_ERROR, self.writer)
                else:
                    await self.break_out_frame(0, StreamErrorCode.PROTOCOL_ERROR, self.writer)
                continue
    ...
    async def consume_complete_stream(self):
        try:
            ...
            print(e)
            print('ASH consume_complete_stream')
